...
export function createUser(payload: Omit<StoredUser, 'createdAt'>): { ok: true, user: StoredUser } | { ok: false, error: string } {
    ...
    try {
        if (payload.birthdate) {
            ...
            if (!isNaN(bd.getTime())) {
                ...
                // birthday student free cake: if registering on birthday and institutional email
                const isBirthday = bd.getDate() === today.getDate() && bd.getMonth() === today.getMonth();
                if (isBirthday && String(payload.email || '').toLowerCase().endsWith('@duoc.cl')) {
                    freeCake = true;
                }
            }
        }
    ...
    const user: StoredUser = { ...payload, discountPercent: discount, lifetimeDiscount: lifetime, freeCakeVoucher: freeCake, freeCakeRedeemed: false, createdAt: new Date().toISOString() };
    users.push(user);
    writeUsers(users);
    return { ok: true, user };
}

export default { readUsers, writeUsers, findUserByEmail, createUser };
